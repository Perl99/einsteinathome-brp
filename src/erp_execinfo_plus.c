...
int backtrace_symbols_fd_plus(const char *const *symbols, int size, int fd)
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        const char *fnEnd = strrchr(symbols[i], ' ');
        /* find funtion+offset delimiters */
        const char *firstP = strrchr(symbols[i], '(');
        const char *secondP = strrchr(symbols[i], ')');
        /* find address delimiters */
        const char *firstB = strrchr(symbols[i], '[');
        const char *secondB = strrchr(symbols[i], ']');
